# Script to run WW3 model, either a 6h forecast or 48h forecast originally developed by Leah Cicon 2020, to run the forecast
# for the NEP. This code was modified to include water levels in the model.
...
export PATH=$PATH:/home/beckyb/projects/def-gemmrich/beckyb/Coastal_ww3/WW3_wlv/WW3/WW3/model/bin:/home/beckyb/projects>
...
"$PATHEXE"/ww3_prnc > out_ww3_prnc
rm ${dates[i]:0:8}${dates[i]:9:2}.nc
rm ww3_prnc.nml #Clear again for the next date

. write_prnc_wlv.sh "$start" "$end"
# Water level forcing
cp $PATHIN/exterp_wlv/${dates[i]:0:8}${dates[i]:9:2}_int_wlv.nc ./${dates[i]:0:8}${dates[i]:9:2}_wlv.nc
echo "Levels input file: "${dates[i]:0:8}${dates[i]:9:2}_wlv.nc
"$PATHEXE"/ww3_prnc > out_ww3_prnc_wlv
rm ${dates[i]:0:8}${dates[i]:9:2}_wlv.nc
#rm ww3_prnc.nml #Clear the inp file then write to it for winds. They cannot be read at the same time.
# Run model
...
mv *_wlv.nc output_files/ww3.${dates[i]:0:8}${dates[i]:9:2}_wlv.nc
# restart file
